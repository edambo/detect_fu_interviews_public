...
```{r}
#| warning: false
general_health <- readRDS(here("data", "cleaned_rds_files", "general_health_import.rds"))

participant <- readRDS(here("data", "unique_id_creation", "participant_unique_id.rds"))

soc_dem <- readRDS(here("data", "cleaned_rds_files", "sociodemographic_information_import.rds"))  

self_report <- readRDS(here("data", "cleaned_rds_files", "self_report_import.rds")) 

sr_aggregate <- readRDS(here::here("data", "self_report_aggregate_data.rds"))

aps <- readRDS(here("data", "cleaned_rds_files", "aps_investigations_import.rds"))   

lpa <- readRDS(here("data", "cleaned_rds_files", "lead_panel_assessment_import.rds")) 
...
```{r}
# Variable labels
all_labels <- read.csv(here::here("exploratory", "ucla_variable_labels.csv"))
```
...
ls_vars_num <- c("ls_lack_companionship_3cat", "ls_feel_left_out_3cat", 
             "ls_feel_isolated_3cat")

ls_vars_f <- c("ls_lack_companionship_3cat_f", "ls_feel_left_out_3cat_f", 
...
```{r}
ls_mp <- missing_pattern(ls, ls_vars_f, all_labels)
ls_mp
```

```{r}
ls_ms <- missing_summary(ls, ls_vars_f, all_labels)
ls_ms
```
...
```{r}
ls_collected_mp <- missing_pattern(ls_collected, ls_vars_f, all_labels)
ls_collected_mp
```

```{r}
ls_collected_ms <- missing_summary(ls_collected, ls_vars_f, all_labels)
ls_collected_ms
```

```{r}
ls <- ls %>%
  drop_na(all_of(ls_vars_f)) %>%
  # Create total score and loneliness determination variables
  mutate(
    ls_total = ls_lack_companionship_3cat + ls_feel_left_out_3cat + 
                   ls_feel_isolated_3cat
  ) %>% select(-c(starts_with("ls") & ends_with("4cat")))
...
```{r}
first_ucla <- ls %>% group_by(unique_id) %>%
  arrange(as.POSIXct(x_created_timestamp), .by_group = TRUE) %>%
  mutate(
    visit_no = row_number()
  ) %>% filter(visit_no == 1) %>%
  ungroup() %>%
  select(medstar_id, all_of(ls_vars_f), ls_total)
```
...
gds <- general_health %>% 
  ...
  # drop_na(c(starts_with("depres") & ends_with("_2cat"))) %>%     
  mutate(
...
following self report variables will also be droppe
