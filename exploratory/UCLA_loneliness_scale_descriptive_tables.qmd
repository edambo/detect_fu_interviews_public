...
unique_people <- gen_part %>% select(name_first, name_last, dob, sex_3cat_f, house_number) %>% group_by_all() %>% summarise(count = n()) %>% unique()
...
sum_table <- data.frame(Value = c("Unique people", "Occur multiple times", "Occur only once", "Total number of rows"),
