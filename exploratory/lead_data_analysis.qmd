...
Load the participant, self report, LEAD panel assessment, LEAD panel meeting attendance, observational measures and sociodemographic information data sets

```{r}
socio_demo_path <- here::here("data", "cleaned_rds_files", 
                              "sociodemographic_information_import.rds")

obs_meas_path <- here::here("data", "cleaned_rds_files", 
...
lpa_attendance_path <- here::here("data", 
                                  "lead_panel_meeting_attendance_reports", 
                                  "combined_LEAD_panel_meeting_attendance.xlsx")
```
...
```{r}
socio_demo <- readRDS(socio_demo_path)
observational_measures <- readRDS(obs_meas_path)
lead_panel_assessment <- readRDS(lpa_path)
participant <- readRDS(participant_path)
lead_attendance <- read_excel(lpa_attendance_path)
```
...
source(here::here("r", "add_shade_column.R"))
```
...
parti <- participant |>
  select(
    ...
    x_unusual_odor_4cat_f, x_unmet_needs_4cat_f, x_difficulty_medications_4cat_f, 
    x_hoarding_medications_4cat_f, x_disarray_hoarding_4cat_f, 
    x_health_safety_concern_4cat_f, x_caregiver_lack_knowledge_4cat_f, 
    x_caregiver_unengaged_4cat_f, x_caregiver_frustrated_4cat_f, 
    x_caregiver_anxious_4cat_f, x_isolated_home_4cat_f, x_depressed_4cat_f, 
...
soc_dem <- socio_demo %>%
  select(medstar_id, sode_race_6cat_f, sode_hispanic_2cat_f, sode_income_7cat_f)
```
...
obs_meas <- observational_measures %>%
  select(medstar_id, at_physical_2cat_f, at_sexual_2cat_f, at_emotional_2cat_f, 
         at_neglect_2cat_f, at_abandon_2cat_f, at_financial_2cat_f, 
         at_self_2cat_f)
```
...
```{r}
cols <- c("at_physical_2cat_f",  "at_sexual_2cat_f", "at_emotional_2cat_f", 
          "at_neglect_2cat_f", "at_abandon_2cat_f", "at_financial_2cat_f",  
          "at_self_2cat_f")
obs_meas <- obs_meas %>%
  mutate(
    medic_abuse_any = case_when(
      if_any(all_of(cols), ~ . == "Yes") ~ 1,
      if_all(all_of(cols), ~ . == "No") ~ 0,
      TRUE ~ NA
    )
  ) %>%
  mutate(
...
```{r}
# List of lead abuse numeric categorical vote columns
lead_abuse_cols <- c(
...
# List of lead abuse factor vote columns
lead_abuse_cols_f <- paste0(lead_abuse_cols, "_f")

# Select relevant variables from LEAD panel assessment df
lpa <- lead_panel_assessment %>%
  select(medstar_id, panelist_name_9cat_f, panelist_discipline_5cat_f, 
         healthcare_worker_2cat_f, x_created_timestamp, x_assessment_month, 
         x_assessment_year, assessment_type_3cat_f, all_of(lead_abuse_cols), 
         all_of(lead_abuse_cols_f), xc_assessment_screened_2cat_f)
```
...
```{r}
# Add suffixes to list of column names
lead_abuse_cols_t <- paste0(lead_abuse_cols, "_t")
lead_abuse_cols_t_p <- paste0(lead_abuse_cols_t, "_p")
...
pos_votes <- lpa %>%
  ...
  rename_with(
    .cols   = all_of(lead_abuse_cols_t_p),
...
```{r}
# Initial
initial_pos_votes <- final_det %>% 
  filter(assessment_type_3cat_f == "Initial assessment")

# Secondary
secondary_pos_votes <- final_det %>% 
  filter(assessment_type_3cat_f == "Secondary assessment")

# Post-DETECT
post_detect_pos_votes <- final_det %>% 
  filter(assessment_type_3cat_f == "Post-detect assessment")
```
...
```{r}
final_determination <- initial_pos_votes[!initial_pos_votes$medstar_id %in% 
                                           secondary_pos_votes$medstar_id, ]
final_determination <- rbind(final_determination, secondary_pos_votes)
```

## Cases with a positive abuse LEAD Panel Assessment determination for any abuse type.

```{r}
part_id <- participant %>% select(medstar_id, name_full)

any_pos_abuse <- final_determination %>%
  select(medstar_id, assessment_type_3cat_f, abuse_any) %>%
  filter(abuse_any == "Yes")

pos_det <- purrr::reduce(list(part_id, any_pos_abuse), dplyr::right_join, 
                         by = "medstar_id")
...
final_determination <- purrr::reduce(list(final_determination, soc_dem, obs_meas, 
                                          parti), 
                                     dplyr::left_join, by = "medstar_id")
final_det_initial <- purrr::reduce(list(initial_pos_votes, soc_dem, obs_meas, 
                                        parti), 
                                   dplyr::left_join, by = "medstar_id")
final_det_sec <- purrr::reduce(list(secondary_pos_votes, soc_dem, obs_meas, 
                                    parti), dplyr::left_join, by = "medstar_id")
final_det_post <- purrr::reduce(list(post_detect_pos_votes, soc_dem, obs_meas, 
                                     parti), 
                                dplyr::left_join, by = "medstar_id")
```
...
# label(soc_dem$sode_hispanic_2cat_f) <- "Ethnicity (Hispanic)"
# label(soc_dem$sode_race_6cat_f) <- "Race"
# label(soc_dem$sode_income_7cat_f) <- "Household Income"
# 
# # Observational measures variables
# label(obs_meas$at_physical_2cat_f) <- "Medic EM physical abuse assessment"
# label(obs_meas$at_sexual_2cat_f) <- "Medic EM sexual abuse assessment"
# label(obs_meas$at_emotional_2cat_f) <- "Medic EM emotional abuse assessment"
# label(obs_meas$at_neglect_2cat_f) <- "Medic EM neglect assessment"
# label(obs_meas$at_abandon_2cat_f) <- "Medic EM abandonment assessment"
# label(obs_meas$at_financial_2cat_f) <- "Medic EM financial abuse assessment"
# label(obs_meas$at_self_2cat_f) <- "Medic EM self-neglect assessment"
...
label_list <- list(
  physical_abuse_det = "LEAD physical abuse determination",
  sexual_abuse_det = "LEAD sexual abuse determination",
  emotional_psycho_abuse_det = "LEAD emotional-psycho abuse determination",
  neglect_det = "LEAD neglect determination",
  self_neglect_det = "LEAD self-neglect determination",
  financial_exploitation_det = "LEAD financial exploitation determination",
  abandonment_det = "LEAD abandonment determination",
  abuse_any = "LEAD any abuse determination",
  
  # Socio-demographic variables
  sode_hispanic_2cat_f = "Ethnicity (Hispanic)",
  sode_race_6cat_f = "Race",
  sode_income_7cat_f = "Household Income",
  
  # Observational measures variables
  at_physical_2cat_f = "Medic EM physical abuse assessment",
  at_sexual_2cat_f = "Medic EM sexual abuse assessment",
  at_emotional_2cat_f = "Medic EM emotional abuse assessment",
  at_neglect_2cat_f = "Medic EM neglect assessment",
  at_abandon_2cat_f = "Medic EM abandonment assessment",
  at_financial_2cat_f = "Medic EM financial abuse assessment",
  at_self_2cat_f = "Medic EM self-neglect assessment",
  medic_abuse_any = "Medic EM any abuse assessment",
  age = "Age (in years)",
  xc_detect_status_2cat_f = "Abuse determination at initial 911 call"
)

label_df <- stack(label_list) %>% rename(
  label = values,
  var = ind
)

final_det_labels <- label_list[names(label_list) %in%
                                 names(final_determination)]

final_determination <- apply_labels(final_determination, final_det_labels)
```
...
initial_compare <- initial_pos_votes %>%
  select(medstar_id, ends_with("_det"), abuse_any) %>%
...
assessment_compare <- purrr::reduce(list(initial_compare, secondary_compare, 
                                         post_compare), dplyr::left_join, 
                                    by = "medstar_id") %>%
  mutate(
...
lpa <- lpa %>%
  ...
  mutate(
    assessment_date = make_date(
      year = x_assessment_year,
...
panelist_participation <- lpa %>%
  group_by(assessment_date) %>%
  reframe(
    unique_panelist = unique(panelist_name_9cat_f),
    attendance = length(unique(panelist_name_9cat_f))
  )
...
unique_panelist_participation <- panelist_participation %>%
  select(assessment_date, attendance) %>%
  unique()
```
...
meeting_duration <- lead_attendance %>%
...
```{r}
initial_panelists <- lpa %>%
  filter(assessment_type_3cat_f == "Initial assessment") %>%
  ungroup() %>%
  select(panelist_name_9cat_f) %>%
  unique()
secondary_panelists <- lpa %>%
  filter(assessment_type_3cat_f == "Secondary assessment") %>%
  ungroup() %>%
  select(panelist_name_9cat_f) %>%
  unique()
...
```{r}
det_cols <- c(
  "physical_abuse_det", "sexual_abuse_det", "emotional_psycho_abuse_det", 
  "neglect_det", "abandonment_det", "financial_exploitation_det", 
  "self_neglect_det"
)

missing_summary(final_determination, c(cols, det_cols), label_df)
```

## Number and proportion of assessements completed by type of assesement

```{r}
sum_table <- unique_case(lpa, medstar_id, assessment_type_3cat_f) %>%
  flextable()
# Set caption
...
```{r}
participation <- ggplot(aes(x = assessment_date, y = attendance), data = unique_panelist_participation) +
  geom_line() +
  labs(
    title = "Number of Unique Panelists Per Month Over Time ", x = "Time",
...
sec_dissimilar <- assessment_compare %>%
  filter(same_is == 0) %>%
  select(-c(same_ip, same_ii, ends_with("_post")))
```
...
# Create values for the table
phys <- sec_dissimilar %>%
  filter(non_con_phys == 1) %>%
  nrow()
sex <- sec_dissimilar %>%
  filter(non_con_sex == 1) %>%
  nrow()
emo <- sec_dissimilar %>%
  filter(non_con_emo == 1) %>%
  nrow()
neg <- sec_dissimilar %>%
  filter(non_con_neg == 1) %>%
  nrow()
self <- sec_dissimilar %>%
  filter(non_con_self == 1) %>%
  nrow()
fin <- sec_dissimilar %>%
  filter(non_con_fin == 1) %>%
  nrow()
aban <- sec_dissimilar %>%
  filter(non_con_aban == 1) %>%
  nrow()
```
...
```{r}
# Dissimilar cases between the initial and secondary assessments
post_dissimilar <- assessment_compare %>%
  filter(same_ip == 0) %>%
  select(-c(same_is, same_ii, ends_with("_sec")))
```
...
```{r}
# Create values for the table
phys2 <- post_dissimilar %>%
  filter(non_con_phys == 1) %>%
  nrow()
sex2 <- post_dissimilar %>%
  filter(non_con_sex == 1) %>%
  nrow()
emo2 <- post_dissimilar %>%
  filter(non_con_emo == 1) %>%
  nrow()
neg2 <- post_dissimilar %>%
  filter(non_con_neg == 1) %>%
  nrow()
self2 <- post_dissimilar %>%
  filter(non_con_self == 1) %>%
  nrow()
fin2 <- post_dissimilar %>%
  filter(non_con_fin == 1) %>%
  nrow()
aban2 <- post_dissimilar %>%
  filter(non_con_aban == 1) %>%
  nrow()
```
...
init_sec <- lpa %>%
  ...
  select(c(medstar_id, assessment_type_3cat_f, panelist_name_9cat_f, all_of(lead_abuse_cols_f)))
...
panelist_votes <- purrr::reduce(list(sec_panelist_votes, initial_panelist_votes), dplyr::left_join,
  by = c("medstar_id", "panelist_name_9cat_f")
)
...
```{r}
type_perc_change <- function(a_agree, a_type, lead_abuse_factor_col) {
  # Create data frame that will be used to compute the vote change frequency
  # per abuse type for each panelist
  tab1 <- as.data.frame(addmargins(table(panelist_votes$panelist_name_9cat_f,
                                         panelist_votes[[a_agree]]), c(2)))
  ...
  # Create data frame containing the number of initial and 
  # secondary votes per abuse type for each panelist
  tab2 <- as.data.frame(table(
    lpa$panelist_name_9cat_f, lpa$assessment_type_3cat_f,
    lpa[[lead_abuse_factor_col]]
  ))
...
# Sexual abuse
sex_disc <- type_perc_change("sex_agree", "Sexual Abuse", "sexual_abuse_2cat_f")
...
# Neglect
neg_disc <- type_perc_change("neg_agree", "Neglect", "neglect_2cat_f")
...
# Abandonment
aban_disc <- type_perc_change("aban_agree", "Abandonment", "abandonment_2cat_f")
...
```{r}
panelist_vote_agree <- panelist_vote_agreement(lpa)

panelist_vote_agree_shade <- add_shade_column(panelist_vote_agree, abuse_type)

# Create and format a flextable that can be downloaded in a Word document
lpa_dis_votes_flex_table <- flextable(panelist_vote_agree_shade,
...
initial_tab <- table1(
  ~ physical_abuse_det + sexual_abuse_det + emotional_psycho_abuse_det + neglect_det + self_neglect_det +
...
fd_na <- final_determination %>% drop_na(ends_with("_det"))
```
...
race_tab <- table1(~ sode_hispanic_2cat_f + sode_race_6cat_f | abuse_any,
  data = fd_na,
  caption = cap_1
)
...
income_tab <- table1(~ sode_income_7cat_f | abuse_any,
  data = fd_na,
  caption = cap_1
)
...
medic_tab <- table1(~ at_physical_2cat_f + at_sexual_2cat_f + 
                      at_emotional_2cat_f + at_neglect_2cat_f + 
                      at_abandon_2cat_f + at_financial_2cat_f + at_self_2cat_f + 
                      medic_abuse_any,
  data = final_determination,
  caption = cap_1
)
