...
```{r}
socio_demo_path <- here::here("data", "cleaned_rds_files", "sociodemographic_information_import.rds")
obs_meas_path <- here::here("data", "cleaned_rds_files", "observational_measures_import.rds")
...
lpa_attendance_path <- here::here("data", "lead_panel_meeting_attendance_reports", "combined_LEAD_panel_meeting_attendance.xlsx")
```

```{r}
socio_demo <- readRDS(socio_demo_path)
observational_measures <- readRDS(socio_demo_path)
lead_panel_assessment <- readRDS(socio_demo_path)
...
source(here::here("r", "vote_agreement.R"))
...
lpa <- lpa %>% select(-c(multiple_count:remove))
...
type_perc_change <- function(a_agree, a_type) {
  ...
  tab2 <- as.data.frame(table(
    lpa$panelist_name_10cat_f, lpa$assessment_type_3cat_f,
    lpa$physical_abuse_2cat_f
  ))
