...
initial <- error_check(initial_e)

initial %>% filter(multiple_count > 1) 
```
...
secondary <- error_check(secondary_e)

secondary %>% filter(multiple_count > 1)
```

```{r}
# Post-detect assessments

post_detect <- error_check(post_detect_e)

post_detect %>% filter(multiple_count > 1)
```
...
```{r}
# Initial assessment
initial <- fix_error(initial)

# Secondary assessment
secondary <- fix_error(secondary)

# Post-detect assessment
post_detect <- fix_error(post_detect)
```
