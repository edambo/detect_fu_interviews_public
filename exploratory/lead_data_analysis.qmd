...
```{r}
initial_panelists     <- lpa %>% filter(assessment_type_3cat_f == "Initial assessment") %>% ungroup() %>%
  select(panelist_name_10cat_f) %>% unique()
secondary_panelists   <- lpa %>% filter(assessment_type_3cat_f == "Secondary assessment") %>% ungroup() %>%
  select(panelist_name_10cat_f) %>% unique() 
post_panelists        <- lpa %>% filter(assessment_type_3cat_f == "Post-detect assessment")%>% ungroup() %>%
  select(panelist_name_10cat_f) %>% unique() 
```
...
type_perc_change <- function(a_agree, a_type) {
  ...
  tab2 <- as.data.frame(table(lpa$panelist_name_10cat_f, lpa$assessment_type_3cat_f,
