...
Interpreted as: In the initial assessment, which disciplines had a tendency to vote differently from the rest?

```{r}
# Subset cases where the initial final abuse lead determination is different from either the secondary determination or the post-DETECT determination in the assessment_compare data frame
non_congruent <- assessment_compare %>% 
  filter(same_is == 0 | same_ip == 0)
 
# Subset non-congruent cases in the initial data frame
ncd <- initial %>% 
  filter(medstar_id %in% non_congruent$medstar_id) 


# Create a subset data frame that contains the cleaned LEAD data for only the initial assessment cases and indicates whether or not the abuse votes were the same. 
# Determine which MedStar IDs did not have voter congruence by comparing the number of observations when the data is grouped by medstar_id and the abuse columns to when the data is only grouped by medstar_id.

initial_non_con <- lead_cleaned %>% filter(assessment_type_3cat_f == "Initial assessment") %>% 
  group_by(
  ...
  mutate(
    congruent_votes = case_when(
      identical_length == voter_length  ~ 1,
      identical_length != voter_length  ~ 0
  
    )
  ) %>%
  filter(congruent_votes == 0)
