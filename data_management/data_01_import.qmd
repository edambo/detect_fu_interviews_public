...
```{r}
username <- key_list("detect_fm_db_readonly")[1,2]

if (Sys.info()['nodename'] == "LAPTOP-PHT7R4VM") {
  driver <- "FileMaker ODBC"
} else if (Sys.info()['nodename'] == "SPH8656") {
  driver <- "/Library/ODBC/FileMaker ODBC.bundle/Contents/MacOS/FileMaker ODBC"
} else {
  stop("You are using an unrecognized computer.")
}
con <- dbConnect(
  odbc(), 
  driver   = "FileMaker ODBC",
  ...
  database = "DETECT",
  uid      = username,
  pwd      = key_get("detect_fm_db_readonly", username)
)
```
...
walk(
  # List of tables to import
      c("APSInvestigations", "SociodemographicInformation", "GeneralHealth", 
        "ClutterScale", "SelfReport", "ObservationalMeasures"),
