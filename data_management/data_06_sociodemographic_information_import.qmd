...
Create factor variables and match levels to codebook for all categorical variables except sode_race.
...
```{r}
soc_dem <- soc_dem %>% select(-c("sode_marital" : "sode_hispanic", "sode_school" : "hsi_contact", 
                                 "sogi_identity", "sogi_orientation", "sode_people_def"))
```

### Re-order columns

```{r}
sociodemographic_information <- soc_dem %>% relocate(c("american_indian_or_alaska_native" : "other"), .after = sode_race)
```
