...
vec_list <- list(aps_id = aps, cls_id = cls, glh_id = glh, lpa_id = lpa, obs_id = obs, par_id = par, sfr_id = sfr, soc_id = soc)
...
```{r}
intersect_detect <- Reduce(intersect, list(aps_id, cls_id, glh_id, lpa_id, obs_id, par_id, sfr_id, soc_id))
...
detect_fu_merge <-lst(par = par_merge, aps = aps_merge, cls = cls_merge, glh = glh_merge, lpa = lpa_merge, obs = obs_merge, sfr = sfr_merge, soc = soc_merge) %>%
  imap(
    function(x, y) x %>% 
