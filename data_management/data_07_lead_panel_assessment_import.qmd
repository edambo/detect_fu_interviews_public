...
lpa <- lpa %>%
  mutate(
    panelist_name_10cat_f = factor(
      panelist_name,
      levels = c("Hannah Bell", "Janice Knebl", "Jason Burnett", 
...
```{r}
lpa <- lpa %>% select(-c(panelist_name, assessment_type : self_neglect, xc_assessment_screened, panelist_discipline, healthcare_worker))
```
