...
```{r}
general_health_import.csv <- file.choose()
general_health <- read_csv(general_health_import.csv)
```
...
```{r}
colnames(general_health)[62]  <- "xc_case_id"
```

## Remove raw variables that have recoded forms

```{r}
general_health<-general_health %>% select(-c(9:47,49:55,59:61,63:65))
```

# Save as rds file

```{r}
saveRDS(general_health, "general_health_import.rds")
