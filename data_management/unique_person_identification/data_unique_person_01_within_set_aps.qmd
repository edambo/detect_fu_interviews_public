...
# 2024-09-19: APS data imported with 568,562 rows and 11 columns.
...
usps_zip <- usps_zip |>
 ...
 # 2024-09-19: USPS ZIP data imported with 44,153 rows and 4 columns.
...
informative_df_import("county_list", county_path)
...
county_list <- unique(tolower(county_list$county_name))
 ...
 # 2024-09-19: COUNTY LIST data imported with 254 rows and 5 columns.
...
informative_df_import("cities", city_path, show_col_types = FALSE)

cities <- cities |>
  # Convert all values to lowercase
  dplyr::mutate(dplyr::across(dplyr::everything(), ~tolower(.x)))

 # 2024-09-19: CITIES data imported with 3,328 rows and 2 columns.
...
point_fixes <- point_fixes |>
 ...
 # 2024-09-28: POINT FIXES data imported with 2 rows and 3 columns.
 # Data last modified on OneDrive: 2024-09-28 18:19:31  
```
...
We first carefully copied modified values into our subset-specific notes column. We excluded any unresolvable entries (which would not receive further modification) and a single street name which was not actually a county road (but would be caught in the regular expression to standardize these roads).
...
aps <- aps |>
  ...
  dplyr::mutate(
    ...
    client_msc = ifelse(
      ...
      stringr::str_detect(client_street_address, '(hwy 7|cr nw|street 60)')
      ) & 
        # Exclude specific street name which is being excluded from standardization
  ...
  dplyr::mutate(
    ...
    client_street_address = ifelse(
      ...
      stringr::str_detect(client_street_address, '(hwy 7|cr nw|street 60)')
        ...
        # Exclude specific street names which are being excluded from 
        # standardization
        !stringr::str_detect(
          client_street_address, 
          '(old county|old country|lake co rd)'
          ),
...
We then turned towards street addresses which did not meet expected formatting. First, we looked at the 520 entries which lacked a numeral, without being a missing value.
