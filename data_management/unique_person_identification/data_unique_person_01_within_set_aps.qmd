...
We isolated the 16 entries that matched our fourth regular expression.
...
We further narrowed our manual verification subset, so that it only included values that continued to have commas in the street address field. The 87 remaining observations were successfully isolated.
...
We purged our checking subset and modified subet for memory management.
...
```{r}
aps <- dplyr::bind_rows(
  aps, dplyr::bind_rows(modified_set, modified_set2)
  )
```
...
#### Any Remaining Non-Standard Characters

We checked for any other remaining non-standard characters (anything other than a letter, number, white space, or hyphen. We isolated the 1,678 observations that contained a non-standard character in the street address field for manual review.
...
There were 257 entries which contained a period (.) character.
...
The 17 entries that started with the symbol were isolated for manual review.
...
We checked for any other remaining non-standard characters (anything other than a letter, number, white space, or hyphen. We isolated the ### observations that contained a non-standard character in the street address field for manual review.
