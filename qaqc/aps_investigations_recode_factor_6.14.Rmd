...
aps_investigations <- read_csv("APS Investigations.csv")
...
Encoding(aps_investigations$aps_first) <- "ASCII"


aps_investigations <- aps_investigations %>%
  mutate(
    aps_first = 
...
aps_investigations <- aps_investigations %>%
  mutate(
    ...
    aps_first_8cat_f = factor(
      aps_first,
      levels = c("10 or more years ago", "5-10 years ago", "1-5 years ago",
                 "In the past year but more than a month ago",
                 "In the past month" , "In the past week", "Don't know",
                 "Refused")),
...
```{r}
write_csv(aps_investigations, "aps_investigations_factor.csv")
