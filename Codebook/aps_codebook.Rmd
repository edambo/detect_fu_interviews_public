---
title: "CodebookR APS Investigations data from recoded data"
...
aps <- read_rds("~.../detect_public/data/aps_factor.rds") #595 23
```
...
for(i in admin_vars) {
  attr(aps[[i]], "source") <- "Administrative data"
}
```
...
aps_vars <- aps %>% 
  select(12:15) %>% # Choose the variables from the raw data
  names()

for(i in aps_vars) {
  attr(aps[[i]], "source") <- "DETECT Follow-up Interview APS Investigations"
}
```
...
factor_vars <- aps %>% 
  select(16:23) %>%  #Choose the recoded variables
  names()

for(i in factor_vars) {
  attr(aps[[i]], "source") <- "Recoded from raw data"
}
```
...
aps_codebook <- codebook(
  df = aps,
  title = "APS Investigations Dataset",
  description = "Data collected during DETECT follow-up interviews on any visits the older adult may have had from Adult Protective Services (APS)",
  no_summary_stats = admin_vars)
```

```{r}
print(aps_codebook, "basic_aps_codebook.docx")
```
